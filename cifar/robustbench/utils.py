...
def _safe_load_state_dict(model: nn.Module, model_name: str,
    ...
    # Hongyeop's code for LoRA or LoHA
    # new_dict = dict()
    # for key, value in state_dict.items():
    #     if key.startswith('block') and key.split('.')[-2].startswith('conv'):
    #         tmp = key.split('.')
    #         tmp = tmp[:-1] + ['conv', 'weight']
    #         tmp = '.'.join(tmp)
    ...
    try:
        #model.load_state_dict(new_dict, strict=False) # True # (state_dict, strict=True)
...
def clean_accuracy(model: nn.Module,
                   #model_anchor: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   #x_s: torch.Tensor, # for src
                   #y_s: torch.Tensor, # for src
                   batch_size: int = 100,
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            y_curr = y[counter * batch_size:(counter + 1) *
                       batch_size].to(device)
            # for src
            #x_src = x_s[counter * batch_size:(counter + 1) *
            #           batch_size].to(device)
            #y_src = y_s[counter * batch_size:(counter + 1) *
            #           batch_size].to(device)

            output = model(x_curr)
            # src_output = model_anchor(x_src)

            acc += (output.max(1)[1] == y_curr).float().sum()

    return acc.item() / x.shape[0] #, outputs, labels, src_outputs, src_labels
